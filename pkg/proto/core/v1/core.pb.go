// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.31.0
// 	protoc        (unknown)
// source: core/v1/core.proto

package corev1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type DataProductType int32

const (
	DataProductType_UNDEFINED DataProductType = 0
	DataProductType_KEY_VALUE DataProductType = 1
	DataProductType_OBJECT    DataProductType = 2
	DataProductType_STREAM    DataProductType = 3
)

// Enum value maps for DataProductType.
var (
	DataProductType_name = map[int32]string{
		0: "UNDEFINED",
		1: "KEY_VALUE",
		2: "OBJECT",
		3: "STREAM",
	}
	DataProductType_value = map[string]int32{
		"UNDEFINED": 0,
		"KEY_VALUE": 1,
		"OBJECT":    2,
		"STREAM":    3,
	}
)

func (x DataProductType) Enum() *DataProductType {
	p := new(DataProductType)
	*p = x
	return p
}

func (x DataProductType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (DataProductType) Descriptor() protoreflect.EnumDescriptor {
	return file_core_v1_core_proto_enumTypes[0].Descriptor()
}

func (DataProductType) Type() protoreflect.EnumType {
	return &file_core_v1_core_proto_enumTypes[0]
}

func (x DataProductType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use DataProductType.Descriptor instead.
func (DataProductType) EnumDescriptor() ([]byte, []int) {
	return file_core_v1_core_proto_rawDescGZIP(), []int{0}
}

type DataProduct struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Name   string          `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Domain string          `protobuf:"bytes,2,opt,name=domain,proto3" json:"domain,omitempty"`
	Type   DataProductType `protobuf:"varint,3,opt,name=type,proto3,enum=core.v1.DataProductType" json:"type,omitempty"`
	// If DataProductType is set to SOURCE, the type of source_data_products
	// must match the type of this DataProduct. Application logic should enforce this.
	SourceDataProducts []*DataProduct `protobuf:"bytes,4,rep,name=source_data_products,json=sourceDataProducts,proto3" json:"source_data_products,omitempty"`
}

func (x *DataProduct) Reset() {
	*x = DataProduct{}
	if protoimpl.UnsafeEnabled {
		mi := &file_core_v1_core_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DataProduct) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DataProduct) ProtoMessage() {}

func (x *DataProduct) ProtoReflect() protoreflect.Message {
	mi := &file_core_v1_core_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DataProduct.ProtoReflect.Descriptor instead.
func (*DataProduct) Descriptor() ([]byte, []int) {
	return file_core_v1_core_proto_rawDescGZIP(), []int{0}
}

func (x *DataProduct) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *DataProduct) GetDomain() string {
	if x != nil {
		return x.Domain
	}
	return ""
}

func (x *DataProduct) GetType() DataProductType {
	if x != nil {
		return x.Type
	}
	return DataProductType_UNDEFINED
}

func (x *DataProduct) GetSourceDataProducts() []*DataProduct {
	if x != nil {
		return x.SourceDataProducts
	}
	return nil
}

var File_core_v1_core_proto protoreflect.FileDescriptor

var file_core_v1_core_proto_rawDesc = []byte{
	,
}

var (
	file_core_v1_core_proto_rawDescOnce sync.Once
	file_core_v1_core_proto_rawDescData = file_core_v1_core_proto_rawDesc
)

func file_core_v1_core_proto_rawDescGZIP() []byte {
	file_core_v1_core_proto_rawDescOnce.Do(func() {
		file_core_v1_core_proto_rawDescData = protoimpl.X.CompressGZIP(file_core_v1_core_proto_rawDescData)
	})
	return file_core_v1_core_proto_rawDescData
}

var file_core_v1_core_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_core_v1_core_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_core_v1_core_proto_goTypes = []interface{}{
	(DataProductType)(0), // 0: core.v1.DataProductType
	(*DataProduct)(nil),  // 1: core.v1.DataProduct
}
var file_core_v1_core_proto_depIdxs = []int32{
	0, // 0: core.v1.DataProduct.type:type_name -> core.v1.DataProductType
	1, // 1: core.v1.DataProduct.source_data_products:type_name -> core.v1.DataProduct
	2, // [2:2] is the sub-list for method output_type
	2, // [2:2] is the sub-list for method input_type
	2, // [2:2] is the sub-list for extension type_name
	2, // [2:2] is the sub-list for extension extendee
	0, // [0:2] is the sub-list for field type_name
}

func init() { file_core_v1_core_proto_init() }
func file_core_v1_core_proto_init() {
	if File_core_v1_core_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_core_v1_core_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DataProduct); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_core_v1_core_proto_rawDesc,
			NumEnums:      1,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_core_v1_core_proto_goTypes,
		DependencyIndexes: file_core_v1_core_proto_depIdxs,
		EnumInfos:         file_core_v1_core_proto_enumTypes,
		MessageInfos:      file_core_v1_core_proto_msgTypes,
	}.Build()
	File_core_v1_core_proto = out.File
	file_core_v1_core_proto_rawDesc = nil
	file_core_v1_core_proto_goTypes = nil
	file_core_v1_core_proto_depIdxs = nil
}
